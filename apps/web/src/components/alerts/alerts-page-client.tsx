...
import { Plus, Mail, MessageSquare, Webhook, Edit } from 'lucide-react';
...
export function AlertsPageClient({ orgId, channels, rules, monitors }: AlertsPageClientProps) {
  ...
  return (
    <>
          ...
          <SaturnCardContent>
            {channels.length === 0 ? (
              <div className="text-center py-8 text-[rgba(55,50,47,0.80)] font-sans">
                ...
                <p className="text-sm text-[rgba(55,50,47,0.60)]">Add an email, connect Slack, or configure a webhook to receive alerts.</p>
              </div>
            ) : (
              <SaturnTable>
                <SaturnTableHeader>
      ...
      <RuleModal
        ...
        orgId={orgId}
        monitors={monitors}
        channels={channels.map(c => ({ id: c.id, label: c.label, type: c.type }))}
      />
      <ChannelModal
