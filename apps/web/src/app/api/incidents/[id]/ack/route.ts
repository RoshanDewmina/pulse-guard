...
import { updateIncidentMessage, postAcknowledgmentReply } from '@/lib/slack/threading';
  ...
  try {
    ...
    await prisma.incidentEvent.create({
      data: {
        ...
        message: `Acknowledged by ${session.user.email}`,
        metadata: {
          userId: session.user.id,
        },
      },
    ...
    if (updated.slackMessageTs && updated.slackChannelId) {
      ...
      if (slackChannel) {
        ...
        if (config.accessToken) {
          ...
          postAcknowledgmentReply(config.accessToken, updated, session.user.id).catch(err => {
