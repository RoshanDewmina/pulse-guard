  ...
  build:
    ...
    steps:
      ...
      - name: Run Build with Monitoring
        ...
        run: |
          pulse run --monitor ${{ secrets.SATURN_MONITOR_ID }} -- make build
```
...
nightly-tests:
  ...
  script:
    - pulse run --monitor $SATURN_MONITOR_ID -- make test
  variables:
  ...
  build-and-test:
    ...
    steps:
      ...
      - run:
          ...
          command: |
            export PATH="$HOME/.saturn/bin:$PATH"
            pulse run --monitor $SATURN_MONITOR_ID -- npm test
```
...
steps:
  ...
  - script: |
      export PATH="$HOME/.saturn/bin:$PATH"
      pulse run --monitor $(SATURN_MONITOR_ID) -- make build
...
```bash
pulse run --monitor YOUR_MONITOR_ID --token YOUR_ORG_TOKEN -- command
```
